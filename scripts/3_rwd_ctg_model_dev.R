...
## find optimal models based on tuning data:
idx_opt_ttt_glmnet <- data_tune_ttt_glmnet$idx_hp[which.max(data_tune_ttt_glmnet$auc_tune)]
idx_opt_ttt_ranger <- data_tune_ttt_ranger$idx_hp[which.max(data_tune_ttt_ranger$auc_tune)]


## only evaluate best (ranger) model:
data_eval_ttt_1 <- tuned_ttt_ranger$archive$predictions(idx_opt_ttt_ranger)[[2]] %>% 
  as.data.table() 
save_rds("data_eval_ttt_1")  

## active comparator setting - best ranger vs. best glmnet model:
ttt_part1 <- tuned_ttt_glmnet$archive$predictions(idx_opt_ttt_glmnet)[[2]] %>% as.data.table() 
names(ttt_part1)[3:5] <- paste0(names(ttt_part1)[3:5], "_glmnet")

ttt_part2 <- tuned_ttt_ranger$archive$predictions(idx_opt_ttt_ranger)[[2]] %>% as.data.table() 
...
tmp_part1_list <- tuned_tmp_glmnet$archive$predictions(idx_opt_ttt_glmnet) 
tmp_part2_list <- tuned_tmp_ranger$archive$predictions(idx_opt_ttt_ranger)
