...
## dataset dimensions (rows: observations, cols: variables):
dim(data_ctg)
...
table(data_ctg$status)
barplot(table(data_ctg$status))
```
...
Potential binary classifiers to investigate:

```{r}
mlr_learners$get("classif.log_reg")
mlr_learners$get("classif.glmnet") # -> focus later on...
mlr_learners$get("classif.ranger") # -> focus later on...
...
Create a evaluation data.frame, i.e. a matrix with two columns: the true labels and the model predictions for the test data. 
...
Simple data splitting in mlr3:

```{r}
set.seed(123)

train_set <- sample(task_ctg$row_ids, 0.7 * task_ctg$nrow)
test_set <- setdiff(task_ctg$row_ids, train_set)
...
```{r}
learner$train(task_ctg, row_ids = train_set)
learner$model %>% str()
```
...
```{r}
pred_train <- learner$predict(task_ctg, row_ids=train_set)
pred_test <- learner$predict(task_ctg, row_ids=test_set)
```
